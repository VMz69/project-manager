  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <header className="mb-10">
        ...
        <p className="text-slate-500 mt-2">Bienvenido, {user?.name || "Invitado"}.</p>
      </header>

      {isManager ? (
        <section className="mb-12 bg-slate-50 p-6 rounded-2xl border border-slate-200">
          <h2 className="text-sm font-bold text-slate-400 uppercase mb-4">Nuevo Proyecto (Panel Manager)</h2>
          <form onSubmit={handleCreate} className="flex flex-col md:flex-row gap-3">
            ...
            <input 
              ...
              required 
            />
            <button type="submit" className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-800 transition-all">
              Añadir
            </button>
          </form>
        </section>
        ...
        <div className="mb-12 p-4 bg-blue-50 text-blue-700 text-sm rounded-xl border border-blue-100 italic">
          * Tu cuenta no tiene permisos de Manager para crear nuevos proyectos.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.length > 0 ? (
          projects.map(p => (
            <ProjectCard key={p.id} project={p} onProjectDeleted={() => setProjects(getProjects())} />
          ))
        ) : (
          <p className="col-span-full text-center text-slate-400 py-10">No hay proyectos para mostrar.</p>
        )}
      </div>
    </div>
  );
